				...
				<c:forEach var="i" begin="${p.beginPerPage }" step="1" end="${p.endPerPage}">
					...
					<div class="row  d-flex" align="center">
						...
						<div class="col-md-2" style="margin-left: 40px;">
								<a href="StoreProductDetail.st?product_num=${list[i].product_num}" type="button" class="myButton" style="width: 50%">상세보기${list[i].product_num }</a>
						</div>
					</div>
				<hr></c:if>
				</c:forEach>
					...
					<ul class="pagination">
       					<c:if test="${p.prev }">
           					 <li class="page-item previous">
              					  <a class="page-link" href="StoreProductListAction.st?page=true&nowPage=${p.startPage-1 }">Previous</a>
           					 </li>
        				</c:if>
				        <c:forEach var="num" begin="${p.startPage }" step="1" end="${p.endPage }">
				            <li class="page-item">
				                <a class="page-link ${p.cri.nowPage == num ? 'active':''}" href="StoreProductListAction.st?page=true&nowPage=${num }">${num }</a>
				            </li>
				        </c:forEach>
				        <c:if test="${p.next }">
				            <li class="page-item next">
				                <a class="page-link" href="StoreProductListAction.st?page=true&nowPage=${p.endPage + 1 }">Next</a>
				            </li>
				        </c:if>
   					 </ul>
				</div>
