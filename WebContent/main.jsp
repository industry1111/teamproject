...
<script type="text/javascript">
	...
	function report_btn_click(product_num) {
		child = window.open("Report.st?product_num="+product_num,"child","width=750,height=730");
	}
	
	function report_login_btn(product_num) {
		if(confirm("로그인이 필요한 서비스 입니다. 로그인 하시겠어요?")){
			window.open("ReportLogin.st?product_num="+product_num,"child","width=750,height=730");
		}
	}
				...
				<c:forEach var="i" begin="${p.beginPerPage }" step="1" end="${p.endPerPage}">
					...
					<c:forEach var="j" begin="0" step="1" end="${slist.size()-1 }">
						<c:if test="${plist[i].member_num eq slist[j].member_num}">
							<c:set var="store_num" value="${slist[j].store_num }"/>
						</c:if>
					</c:forEach>
					<hr>
					<div class="row">
						...
						<br>
							...
							<c:choose>
								<c:when test="${member_num eq 0 }">
									...
									<a id="report"	style="color: black;" onclick="report_login_btn(${plist[i].product_num});">	
								...
								<c:otherwise>
									<c:if test="${jlist.size() > 0 }">
											<c:forEach var="k" begin="0" step="1" end="${jlist.size()-1 }">
												<c:if test="${store_num eq jlist[k].store_num }">
													<c:set var="j_store_num" value="${jlist[k].store_num}"/>	
												</c:if>
											</c:forEach>	
									</c:if>
									...
									<c:if test="${loop2}">
										<c:if test="${store_num eq j_store_num}">
											<a id="jjim"  class="jjim2 check" value="${store_num}" style="color: red;"> 
												<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-suit-heart" viewBox="0 0 16 16">
									  				<path d="m8 6.236-.894-1.789c-.222-.443-.607-1.08-1.152-1.595C5.418 2.345 4.776 2 4 2 2.324 2 1 3.326 1 4.92c0 1.211.554 2.066 1.868 3.37.337.334.721.695 1.146 1.093C5.122 10.423 6.5 11.717 8 13.447c1.5-1.73 2.878-3.024 3.986-4.064.425-.398.81-.76 1.146-1.093C14.446 6.986 15 6.131 15 4.92 15 3.326 13.676 2 12 2c-.777 0-1.418.345-1.954.852-.545.515-.93 1.152-1.152 1.595L8 6.236zm.392 8.292a.513.513 0 0 1-.784 0c-1.601-1.902-3.05-3.262-4.243-4.381C1.3 8.208 0 6.989 0 4.92 0 2.755 1.79 1 4 1c1.6 0 2.719 1.05 3.404 2.008.26.365.458.716.596.992a7.55 7.55 0 0 1 .596-.992C9.281 2.049 10.4 1 12 1c2.21 0 4 1.755 4 3.92 0 2.069-1.3 3.288-3.365 5.227-1.193 1.12-2.642 2.48-4.243 4.38z"/>
												</svg>찜&nbsp;·
											</a>
										</c:if>
										<c:if test="${store_num ne j_store_num}">
											<a id="jjim"  class="jjim" value="${store_num}" style="color:black;">
													<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-suit-heart" viewBox="0 0 16 16">
									  					<path d="m8 6.236-.894-1.789c-.222-.443-.607-1.08-1.152-1.595C5.418 2.345 4.776 2 4 2 2.324 2 1 3.326 1 4.92c0 1.211.554 2.066 1.868 3.37.337.334.721.695 1.146 1.093C5.122 10.423 6.5 11.717 8 13.447c1.5-1.73 2.878-3.024 3.986-4.064.425-.398.81-.76 1.146-1.093C14.446 6.986 15 6.131 15 4.92 15 3.326 13.676 2 12 2c-.777 0-1.418.345-1.954.852-.545.515-.93 1.152-1.152 1.595L8 6.236zm.392 8.292a.513.513 0 0 1-.784 0c-1.601-1.902-3.05-3.262-4.243-4.381C1.3 8.208 0 6.989 0 4.92 0 2.755 1.79 1 4 1c1.6 0 2.719 1.05 3.404 2.008.26.365.458.716.596.992a7.55 7.55 0 0 1 .596-.992C9.281 2.049 10.4 1 12 1c2.21 0 4 1.755 4 3.92 0 2.069-1.3 3.288-3.365 5.227-1.193 1.12-2.642 2.48-4.243 4.38z"/>
													</svg>찜&nbsp;·
											</a>
									...
									<a id="report"	style="color: black;" onclick="report_btn_click(${plist[i].product_num});">	
									<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-emoji-angry" viewBox="0 0 16 16">
		  								<path d="M8 15A7 7 0 1 1 8 1a7 7 0 0 1 0 14zm0 1A8 8 0 1 0 8 0a8 8 0 0 0 0 16z"/>
		 								 <path d="M4.285 12.433a.5.5 0 0 0 .683-.183A3.498 3.498 0 0 1 8 10.5c1.295 0 2.426.703 3.032 1.75a.5.5 0 0 0 .866-.5A4.498 4.498 0 0 0 8 9.5a4.5 4.5 0 0 0-3.898 2.25.5.5 0 0 0 .183.683zm6.991-8.38a.5.5 0 1 1 .448.894l-1.009.504c.176.27.285.64.285 1.049 0 .828-.448 1.5-1 1.5s-1-.672-1-1.5c0-.247.04-.48.11-.686a.502.502 0 0 1 .166-.761l2-1zm-6.552 0a.5.5 0 0 0-.448.894l1.009.504A1.94 1.94 0 0 0 5 6.5C5 7.328 5.448 8 6 8s1-.672 1-1.5c0-.247-.04-.48-.11-.686a.502.502 0 0 0-.166-.761l-2-1z"/>
									</svg>신고하기
