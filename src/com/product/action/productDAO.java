...
public class productDAO {
	...
	public void insertProduct(productDTO pdto) { // 상품 정보를 입력함
		...
		try {
			...
			String sql = "insert into product (member_num,product_name,product_img,category_name,"
					+ "product_price,product_count,product_brand,product_description)" + "values(?,?,?,?,?,?,?,?)";
	...
	public List<productDTO> getProductList(int category_code1, int category_code2, int category_code3, String brand,
		...
		String sql = "select p.product_num, s.store_name, p.member_num, p.product_name, p.product_img, c.category_name,p.product_price,"
				+ " p.product_description, product_regdate, product_brand, s.category_num, c.category_name, s.store_num, "
	...
	public List<productDTO> getProductList(String searchbox,String price1, String price2, String sort, String price) {
		List<productDTO> list = new ArrayList<productDTO>();
		String sql = "select p.product_num, s.store_name, p.member_num, p.product_name, p.product_img, c.category_name,p.product_price,"
				...
				+ " from product p join category c on p.category_name = c.category_name "
				+ " join seller s on s.member_num = p.member_num"
				+ " where (lower(store_name) like lower(?) or lower(product_brand) like lower(?) or lower(product_name) like lower(?))";
	...
	// 브랜드 별 상품 가져오기
	public List<productDTO> getProductInfo(int store_num, String brand) { // store_num에 따라 상품 리스트 가져오기

		List<productDTO> list = new ArrayList<productDTO>();
		try {
			getCon();
			String sql = "select * from seller natural join product where store_num= ? and product_brand = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, store_num);
